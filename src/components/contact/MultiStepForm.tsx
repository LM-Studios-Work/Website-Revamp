...
import { useContactForm, ContactFormData } from "@/src/hooks/useContactForm";
 ...
 * 
 * Large icon-based card for selecting project type
 */
    ...
    >
      <Icon
        className={`w-8 h-8 mb-3 transition-colors ${
          selected ? "text-[#d4f534]" : "text-gray-400 group-hover:text-white"
        }`}
      />
      <span
...
/**
 * PillOption Component
 * 
 * Simple pill-shaped option button for selections
 */
function PillOption({
  label,
  selected,
 ...
 * 
 * A three-step contact form with animated transitions.
 * Uses the useContactForm hook for state management.
 * 
 * Steps:
 * 1. Identity - Contact information
 * 2. Project - Project type and status
 * 3. Scope - Budget, timeline, and details
 */
export function MultiStepForm() {
  const { step, formData, updateData, nextStep, prevStep, submitForm } =
    useContactForm();
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      {/* Form Card */}
      <GlassCard className="p-8 md:p-12 shadow-2xl relative">
        {/* Decorative Glow */}
        ...
        {step === 1 && (
          ...
          >
            ...
            <div className="space-y-3">
              <label className="text-sm text-white/80 ml-1 block">
                Preferred mode of contact
              </label>
              <div className="grid grid-cols-3 gap-4">
                {[
                  { label: "WhatsApp" as const, icon: Smartphone },
                  { label: "Call" as const, icon: Phone },
                  { label: "Email" as const, icon: Mail },
                ].map((method) => (
                  <button
                    key={method.label}
                    type="button"
        ...
        {step === 2 && (
          ...
          >
            ...
            <div className="mb-8">
              <label className="block text-sm text-white/80 ml-1 mb-4">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              ...
              <div className="space-y-4">
                <label className="text-sm text-white/80 ml-1">
                  Budget Range
                </label>
                <div className="flex flex-col gap-3">
                  {["R1,500 - R2,500", "R2,500 - R5,000", "R5,000+", "N/A"].map(
                    (opt) => (
                      <PillOption
                        key={opt}
                        label={opt}
              ...
              {/* Timeline */}
              <div className="space-y-4">
                <label className="text-sm text-white/80 ml-1">Timeline</label>
                <div className="flex flex-col gap-3">
                  {[
                    "ASAP / Rush",
                    "Within 2-4 weeks",
            ...
            {/* Content Readiness */}
            <div className="mb-8">
              <label className="text-sm text-white/80 ml-1 mb-3 block">
                Content Readiness
              </label>
              <select
                ...
                onChange={(e) =>
                  updateData("contentReadiness", e.target.value)
                }
              >
                <option value="" disabled className="text-gray-500">
                  Select an option...
                </option>
                <option className="bg-gray-900" value="ready">
                  I have all text and images ready.
                </option>
                <option className="bg-gray-900" value="help">
                  I need help creating content.
                </option>
                <option className="bg-gray-900" value="working">
